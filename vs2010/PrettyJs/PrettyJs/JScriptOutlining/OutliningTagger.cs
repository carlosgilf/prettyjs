    ...
    sealed class OutliningTagger : ITagger<IOutliningRegionTag>, IDisposable
    {
        ...
        public IEnumerable<ITagSpan<IOutliningRegionTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (var section in _sections)
            {
                ...
                if (spans.IntersectsWith(new NormalizedSnapshotSpanCollection(sectionSpan)))
                {
                    ...
                    var tag = new OutliningRegionTag(isCustom,true,firstLine, collapsedHintText);
        ...
        List<ITrackingSpan> ReparseRange(ITextSnapshot snapshot, int reparseRangeStart, int reparseRangeEnd, out bool unbalanced)
        {
            ...
            for (int i = reparseRangeStart; i < reparseRangeEnd; i++)
            {
                ...
                {
                    var point = new SnapshotSpan(snapshot, i, 1).Start;
                    if (!JScriptEditorUtil.IsClassifiedAs(_classifier, point,JScriptClassifications.Comment))
                    {
                        continue;
                    }
                    if (i + 3 < reparseRangeEnd && snapshot.GetText(i, 4) == "//##")
                    {
                        i = i + 3;
                        customStack.Push(i+1);
                    }
                    else if (i + 5 < reparseRangeEnd && snapshot.GetText(i, 6) == "//#end")
                    {
                        i = i + 5;
                        if (customStack.Count > 0)
                        {
                            int start = customStack.Pop();
            ...
            unbalanced = unbalanced || c_unbalanced;
