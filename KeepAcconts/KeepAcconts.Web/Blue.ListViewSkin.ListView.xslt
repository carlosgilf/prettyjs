...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template name="tplListViewDrawDetailsRows">
    ...
    <div dir="{$dir}" vwgfocuselement="1" tabindex="-1" vwgscrollable="1" onscroll="mobjApp.Controls_StoreScroll(window,'{@Id}',this);mobjApp.Web_SyncScroll(this,mobjApp.Web_GetElementById('HEADER_{@Id}',window),1,'{$dir}')" onresize="mobjApp.Web_SyncScrollSchedule(this,mobjApp.Web_GetElementById('HEADER_{@Id}',window),1,'{$dir}');">
      ...
      <xsl:if test="not(name()='Tags.Group')">
        <div id="VWG_{@Id}" tabindex="-1" vwgevents="{@Attr.Events}" vwgfocuselement="1">
          ...
          <table cellpadding="0" cellspacing="2">
            <tr>
              <td>
                <table cellpadding="0" cellspacing="0" align="center" dir="{$dir}">
                  <tr>
                    <!-- TD of CheckBox-->
                    <xsl:if test="$prmCheckBoxes='1'">
                      <!-- TD of CheckBox -->
  ...
  <xsl:template name="tplListViewDrawHeaders">
    ...
    <table border="0" cellspacing="0" cellpadding="0" style="height:100%;width:100%;table-layout:fixed">
      ...
      <tr>
        <xsl:if test="$prmCheckBoxes='1'">
          <td class="ListView-FontData ListView-HeaderCell">
            <table border="0" cellSpacing="0" cellPadding="0" style="width:100%;height:100%">
              <tr>
                <td align="center" ss="jrt" onclick="mobjApp.ListView_CheckAllClick(this,{@Id},window)">
                  <img id="LVI_CB_ALL_{@Id}"  class="ListView-CheckBoxImage" src="[Skin.Path]CheckBox0.gif.wgx"/>
                </td>
              </tr>
            </table></td>
  ...
  <xsl:template name="tplDrawDetailsRow">
    ...
    <xsl:variable name="varAlternate" select="(position() mod 2)=1"/>
    ...
    <xsl:if test="name()='Tags.Row'">
      <tr tabindex="-1" vwgevents="{@Attr.Events}" vwgfocuselement="1" style="height:{$varHeight}px;">
        <xsl:call-template name="tplSetToolTip"/>
        ...
        <xsl:if test="$prmCheckBoxes='1'">
          ...
          <td class="ListView-Cell ListView-SeperatorCell">&#160;</td>
        </xsl:if>
  ...
  <xsl:template name="tplDrawListViewPaging">
    <table style="height:100%;" border="0" cellpadding="0" cellspacing="0" class="ListView-PagingPanel" dir="{$dir}" align="center">
      <tr>
        <td></td>
        <td >
          <xsl:attribute name="class">ListView-PagingFirstImageWidth<xsl:if test="@Attr.CurrentPage=1"> Common-Disabled</xsl:if></xsl:attribute>
          <xsl:if test="@Attr.CurrentPage=1">
            <xsl:attribute name="disabled">1</xsl:attribute>
          </xsl:if>
          <a onclick="mobjApp.List_NavigateHome('{@Id}')">
            <div class="ListView-PagingFirstButtonStyle">&#160;</div>
          </a>
        </td>
        <td >
          <xsl:attribute name="class">
            ListView-PagingPreviousImageWidth<xsl:if test="@Attr.CurrentPage=1"> Common-Disabled</xsl:if>
          </xsl:attribute>
          <xsl:if test="@Attr.CurrentPage=1">
            <xsl:attribute name="disabled">1</xsl:attribute>
          </xsl:if>
          <a onclick="mobjApp.List_NavigateBack('{@Id}')">
            <div class="ListView-PagingPrevButtonStyle">&#160;</div>
          </a>
        </td>
        <td dir="ltr" width="[Skin.PagingBoxWidth]px" style="padding-bottom:1;"  >
          <div class="Common-FontData ListView-PagingStatusBox">
            <xsl:value-of select="@Attr.CurrentPage"/> / <xsl:value-of select="@Attr.TotalPages"/>        
          </div>
        </td>
        <td>
          <xsl:attribute name="class">
            ListView-PagingNextImageWidth<xsl:if test="@Attr.CurrentPage=@Attr.TotalPages"> Common-Disabled</xsl:if>
          </xsl:attribute>
          <xsl:if test="@Attr.CurrentPage=@Attr.TotalPages">
            <xsl:attribute name="disabled">1</xsl:attribute>
          </xsl:if>
          <a onclick="mobjApp.List_NavigateNext('{@Id}')" >
            <div class="ListView-PagingNextButtonStyle">&#160;</div>
          </a>
        </td>
        <td >
          <xsl:attribute name="class">
            ListView-PagingLastImageWidth<xsl:if test="@Attr.CurrentPage=@Attr.TotalPages"> Common-Disabled</xsl:if>
          </xsl:attribute>
          <xsl:if test="@Attr.CurrentPage=@Attr.TotalPages">
            <xsl:attribute name="disabled">1</xsl:attribute>
          </xsl:if>
          <a onclick="mobjApp.List_NavigateEnd('{@Id}')" >
            <div  class="ListView-PagingLastButtonStyle">&#160;</div>
          </a>
        </td>
        <td></td>
      </tr>
