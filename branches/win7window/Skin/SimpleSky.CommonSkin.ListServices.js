...
function List_Select(strGuid, strSelectedItemID, intSelectionMode, blnUseIndexes, blnSelectIcon, objWindow, blnShiftKey, blnCtrlKey, blnSuspendRaiseEvents, intKeyCode) {
    ...
    if (objNode) {
        ...
        if (blnSelectionChanged) {
            //hack by jinrt:clear select items
...
//jrt for change ItemsPerPage fire event
function PageSize_HandleEvents(strGuid, strAction, objInput, objEvent) {
    var objNode = Data_GetNode(strGuid);
    ...
    if (!Aux_IsNullOrEmpty(strGuid)) {
        ...
        var firecChangeEvent = function () {
            var perPage = Xml_GetAttribute(objNode, "Attr.DisplayYearMonth");
            if (objInput.value == "") {
                objInput.value = perPage;
                return;
            }

            var value = objInput.value * 1;
            if (value > 300) {
                objInput.value = perPage;
                alert("每页显示记录数不能超过300条！");
                return;
            }
            ...
            if (objInput.value != perPage) {
                //var currentPage = Xml_GetAttribute(objNode, "Attr.CurrentPage");

                //change the items per page value
                var objEvent = Events_CreateEvent(strGuid, "ChangePerPage", true);
                Events_SetEventAttribute(objEvent, "ItemsPerPage", objInput.value);

                //fire paging changed event
                List_NavigateTo(strGuid, 1);
            }
        };
            ...
            case "keypress":
                ...
                if (objEvent && objInput) {
                    ...
                    if (!Aux_IsNullOrEmpty(intKeyCode)) {
                        ...
                        blnIsValidKey = (intKeyCode >= 48 && intKeyCode <= 57) ||Web_IsNavigationKey(intKeyCode) ||[mcntBackspaceKey].contains(intKeyCode);
